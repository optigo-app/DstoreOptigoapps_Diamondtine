...
export const GetCount = async (cookies, islogin) => {
  ...
  // const islogin = localStorage.getItem("registerEmail")


  let customerId =
    (storeInit?.IsB2BWebsite == 0 && (islogin == "false" || islogin == "f"))
      ? cookies?.visitorId
      : Customer_id?.id;

  let customerAppUserId = storeInit?.IsB2BWebsite ? cookies?.visiterId : UserEmail
  let EncodeData = { FrontEnd_RegNo: `${storeInit?.FrontEnd_RegNo}`, Customerid: `${customerId}` }

  const encodedCombinedValue = btoa(JSON.stringify(EncodeData));

  let body = {
    "con": `{\"id\":\"\",\"mode\":\"Getcount\",\"appuserid\":\"${ (storeInit?.IsB2BWebsite == 0 && (islogin == "false" || islogin == "f")) ? cookies?.visiterId : UserEmail}\"}`,
